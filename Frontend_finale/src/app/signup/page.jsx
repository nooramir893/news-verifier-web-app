...
import { Shield, Mail, Lock, Eye, EyeOff, User, ArrowRight, CheckCircle } from 'lucide-react'
    ...
    <div className="mt-2.5 sxgbe-y-2">
      <div className="flex items-center gap-2">
        <div className="flex-1 h-1.5 bg-navy-900 rounded-full flex gap-1 overflow-hidden">
          {[0,1,2,3].map(i=>(
            <div
              key={i}
              className={`flex-1 h-full rounded-full transition-all duration-300 ${i<score?bar:'bg-navy-800'}`}
            />
          ))}
        </div>

        <span className={`text-xs font-mono ${clr}`}>
          {lbl}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1">
        {checks.map(c=>(
          <div
            key={c.l}
            className={`flex items-center gap-1.5 text-xs font-body transition-colors ${c.ok?'text-neon':'text-gray-600'}`}
          >
            <CheckCircle size={10}/>
            {c.l}
          </div>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 font-body text-sm">
            Join News Verifier and start detecting fake news
          </p>
          ...
          <form onSubmit={submit} className="sxgbe-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                {form.confirm&&form.confirm===form.password&&(
                  <CheckCircle
                    size={13}
                    className="absolute right-9 top-1/2 -translate-y-1/2 text-neon"
                  />
                )}
            ...
            <div>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <label
                  htmlFor="terms"
                  className="text-sm text-gray-400 font-body cursor-pointer leading-relaxed"
                >
                  I agree to the{' '}
