    ...
    <div className="min-h-screen bg-navy-950 grid-bg flex items-center justify-center px-4 relative">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 font-body text-sm">Sign in to access the AI news detector</p>
        </div>
        ...
        <div className="glass nb rounded-2xl p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-gray-500 text-xs font-mono">or continue with email</span>
            <div className="flex-1 h-px bg-primary/12"/>
          </div>
          ...
          <form onSubmit={submit} className="sxgbe-y-4">
            ...
            <div>
              <div className="flex justify-between mb-1.5">
                ...
                <Link href="#" className="text-xs text-primary hover:text-secondary font-body transition-colors">Forgot password?</Link>
              </div>
              <div className="relative">
                <Lock size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500"/>
                <input type={show?'text':'password'} value={form.password} onChange={e=>set('password',e.target.value)} placeholder="••••••••" className={`${inputCls('password')} pr-11`}/>
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="rem" className="w-4 h-4 accent-primary rounded cursor-pointer"/>
              <label htmlFor="rem" className="text-sm text-gray-400 font-body cursor-pointer">Remember me for 30 days</label>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Signing in...</> : <>Sign In<ArrowRight size={14}/></>}
            </button>
          </form>
        </div>

        <p className="text-center text-gray-500 text-sm font-body mt-5">
          Don't have an account?{' '}<Link href="/signup" className="text-primary hover:text-secondary font-semibold transition-colors">Create one free</Link>
        </p>
